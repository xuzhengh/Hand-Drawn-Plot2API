...
class VisionTransformer(nn.Module):
    ...
    def __init__(self, global_pool=False, img_size=224, patch_size=16, in_chans=3, num_classes=1000, embed_dim=768, depth=12,
        ...
        # 第一步，定义在整个网络中共享的投影矩阵
        self.d_adapt = nn.Parameter(torch.empty(self.tuning_config.d_model, self.tuning_config.ffn_num))
        nn.init.xavier_uniform_(self.d_adapt)
    ...
    def forward_features(self, x):
        ...
        for idx, blk in enumerate(self.blocks):
            ...
            #第二步，将共享的矩阵在使用时传给block
            x = blk(x,adapter_projection=self.d_adapt)
            if self.tuning_config.vpt_on:
    ...
    def forward(self, x):
        # for layer_encoder in self.blocks:
        #     layer_encoder(adapter_projection=self.d_adapt)
