...
class MonaOp(nn.Module):
    def __init__(self, in_features):
        super().__init__()
        self.conv0 = nn.Conv2d(in_features, in_features, kernel_size=3, padding=3 // 2, groups=in_features)
        self.bn0=nn.BatchNorm2d(in_features)
        self.conv1 = nn.Conv2d(in_features, in_features, kernel_size=3, padding=3, groups=in_features,dilation=3)
        # self.conv2 = nn.Conv2d(in_features, in_features, kernel_size=5, padding=5 // 2, groups=in_features)
        # self.conv3 = nn.Conv2d(in_features, in_features, kernel_size=7, padding=7 // 2, groups=in_features)
        # self.projector = nn.Conv2d(in_features, in_features, kernel_size=1, )
        self.se = SqueezeExcite(in_features, 0.25)
        self.conv4 = nn.Conv2d(in_features, int(in_features*0.5), kernel_size=1, )
        self.gelu = nn.GELU()
        self.conv5 = nn.Conv2d(int(in_features*0.5), in_features, kernel_size=1, )
        self.channel_mixer = Residual(nn.Sequential(self.conv4, self.gelu, self.conv5))

    def forward(self, x):
        identity = x
        conv1_x = self.conv1(self.bn0(self.conv0(x)))
